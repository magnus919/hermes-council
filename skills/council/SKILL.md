...
`/council` spawns a panel of **custom-composed expert agents** to debate any question. Unlike a single agent reasoning in monologue, Council produces **genuine multi-perspective analysis** — independent agents with distinct backgrounds, biases, and analytical approaches engage in structured rounds of debate, then produce a decision landscape for the principal to read.
